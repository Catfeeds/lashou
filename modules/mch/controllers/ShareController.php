...
class ShareController extends Controller
{
    ...
    public function actionXiaji(){
        $form = new ShareListForm();
        $form->attributes = \Yii::$app->request->get();
        $form->store_id = $this->store->id;
        $form->limit = 10;
        $list = $form->getsearch();
        $parent = User::findOne(['id'=>\Yii::$app->request->get()['user_id']]);
//        $sql = $list->createCommand()->getRawSql();
//        var_dump($sql);die;
        //var_dump($list[0]);die;
        foreach ((array)$list[0] as $index => $value) {
            $list[0][$index]['addtime'] = date('Y-m-d', $value['addtime']);
            $list[0][$index]['sjnc'] = $parent->nickname;
            $list[0][$index]['parent_id'] = $parent->id;
            $f_children = Share::getChildren($value['id']);
            if (!$f_children){
                $list[0][$index]['first'] = 0;
                continue;
            }

            $list[0][$index]['first'] += count($f_children);

        }
        //var_dump($list);die;
        //$list['nickname'] = $nickname;
        return json_encode(['list'=>($list[0]),'page'=>$list[1]]);
    }
    public function actionSearchxiaji(){
...
//        var_dump($sql);die;
        foreach ($list as $index => $value) {
            $list[$index]['sjnc'] = $parent->nickname;
            $list[$index]['parent_id'] = $parent->id;
            $list[$index]['addtime'] = date('Y-m-d', $value['addtime']);
            $f_children = Share::getChildren($value['id']);
            if (!$f_children){
                $list[$index]['first'] = 0;
                continue;
    ...
    public function actionGetteam(){
        ...
        $form->attributes = \Yii::$app->request->get();
        $form->store_id = $this->store->id;
        $list = $form->getTeam(\Yii::$app->request->get()['user_id']);
        return json_encode($list);
    }
    public function actionGetteam2(){
        $form = new ShareListForm();
        $form->attributes = \Yii::$app->request->get();
        $form->store_id = $this->store->id;
        $list = $form->getTeam(\Yii::$app->request->get()['user_id']);
...
//    添加备注
    ...
    public function actionLog($id = 0)
    {
        ...
        $list = LsBonusLog::find()->where(['uid'=>$user_id])->asArray()->all();
    ...
    public function actionApply($id = 0, $status = 0)
    {
        $cash = Cash::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
        $user_id = $cash->user_id;
        $hongbao = $cash->hongbao;
        $is_cash = Cash::find()->where(['user_id'=>$user_id,'hongbao'=>1])->andWhere(['in','status',[1,2]])->one();
        if(!empty($is_cash)&&$hongbao==1){
            return json_encode([
                'code' => 1,
                'msg' => '该用户红包已打款或有审核成功,请驳回'
            ], JSON_UNESCAPED_UNICODE);
        }
        if (!$cash) {
            return json_encode([
        ...
        \Yii::$app->cache->set('cash_cache_' . 16842, false);
        $cash_cache = \Yii::$app->cache->get('cash_cache_' . $id);
...
//    余额提现
    ...
    public function actionConfirm($id = 0, $status = 0)
    {
        ...
        $user_id = $cash->user_id;
        $hongbao = $cash->hongbao;
        $is_cash = Cash::find()->where(['user_id'=>$user_id,'hongbao'=>1])->andWhere(['in','status',[2]])->one();
        if(!empty($is_cash)&&$hongbao==1){
            return json_encode([
                'code' => 1,
                'msg' => '该用户红包已打款,请驳回'
        ...
        //\Yii::$app->cache->set('cash_cache_' . $id, 16380);
        $cash_cache = \Yii::$app->cache->get('cash_cache_' . $id);
        if (!$cash_cache && $cash_cache == $cash->order_no) {
            return $this->renderJson([
                'code' => 1,
        ...
        if ($status == 2) {  //微信自动打款
            ...
            if($user->is_partner==1){
                ...
                $amount = round(($cash->price * 0.9),2) * 100;
            }
            $data = [
...
//            $cash->type = 2;
    ...
    public function actionBatchcash()
    {
        ...
        $condition = ['and', ['in', 'id', $goods_id_group], ['store_id' => $this->store->id],['is_partner'=>0]];
        if ($get['type'] == 0) { //批量审核

...
class OrderListForm extends Model
{
    ...
    public function rules()
    {
        return [
            [['keyword',], 'trim'],
            [['status','is_recycle', 'page', 'limit', 'user_id', 'is_offline', 'clerk_id', 'shop_id', 'keyword_1','send_type'], 'integer'],
    ...
    public function search()
    {
        ...
        if($this->send_type == "0"){
            $query->andWhere(['or',['o.is_offline'=>0], ['<=','o.partner_id',1]]);
        }
        if($this->send_type == "1"){
            $query->andWhere(['o.is_offline'=>1])->andWhere(['>','o.partner_id',1]);
        }
        //print_r([gettype($this->send_type), 999, intval($this->send_type), in_array($this->send_type, [0, 1])]);
        ...
        if ($this->keyword or $this ->keyword == 0) {//关键字查找
            if($this->keyword_1 == 1){
            ...
            if($this->keyword_1 == 5){
                if($this->status == 1){
                    if(preg_match("/^\d*$/",$this->keyword)){
                        $query->andWhere(['or',['o.partner_id'=>$this->keyword],['o.send_user_id'=>$this->keyword]]);
                    }else{
                        echo "<script>alert('请输入ID')</script>";
                    }
                }else{
                    if(preg_match("/^\d*$/",$this->keyword)){
                        $query->andWhere(['or',['o.partner_id'=>$this->keyword],['o.send_user_id'=>$this->keyword]]);
...
//            ]);
        ...
        foreach ($list as $i => $item) {
            ...
            if ($item['shop_id'] && $item['shop_id'] != 0) {
                $shop = Lashou::getShopByPartner($item['shop_id']);

...
class CashLsForm extends Model
{
    public $store_id;
    public $user_id;
...
//            ->where(['c.is_delete'=>0,'c.store_id'=>$this->store_id])
        ...
        $list = $query->limit($pagination->limit)->offset($pagination->offset)->orderBy('c.status ASC,c.add_time DESC')
            ->select([

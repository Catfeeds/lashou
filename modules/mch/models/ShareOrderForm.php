...
//        )";
        ...
        $sql = "SELECT `al`.*,u.nickname FROM ( {$sql_s}  ) `al` LEFT JOIN {$user_table} `u` ON al.user_id = u.id";
        ...
        if($this->keyword){
            ...
            $andWhere .= " (`al`.order_no LIKE '%{$this->keyword}%' OR u.nickname LIKE '%{$this->keyword}%' OR u.id LIKE '%{$this->keyword}%') ";
        }
        if($this->parent_id){
            if(!empty($andWhere)){
                $andWhere .= " AND ";

...
//            'u.is_delete' => 0
//        ])->andWhere(['NOT', ['wechat_open_id' => '']])->leftJoin(Shop::tableName() . ' s', 's.id=u.shop_id')
//        ->leftJoin(Level::tableName(). ' l','l.level=u.level and l.is_delete = 0 and l.store_id = '.$this->store_id);
        $query = User::find()->alias('u')->where([
            'u.type' => 1,
            'u.store_id' => $this->store_id,
            'u.is_delete' => 0
        ])->andWhere(['NOT', ['wechat_open_id' => '']]);
        if ($this->keyword){
            if($this->search ==1){
                $query->andWhere(['or',['LIKE','u.nickname',$this->keyword],['LIKE','u.id',$this->keyword],['=','u.mobile',$this->keyword]]);

...
class PartnerListOrders extends Model
{
    public $store_id;
    public $user_id;
    public $keyword;
    ...
    public function search()
    {
        ...
        $query = Order::find()->alias('o')->where([
            ...
            'o.mch_id' => 0,
        ])->andWhere(['!=','partner_id',0])->leftJoin(['u' => User::tableName()], 'u.id=o.user_id');
        ...
        if ($this->keyword) {//关键字查找
            ...
            if($this->keyword_1 == 4){//合伙人ID查找订单
                $query->andWhere(['=', 'o.partner_id', $this->keyword]);
            }
//            $query->andWhere([
//                'OR',
...
//            ]);
        ...
        foreach ($list as $i => $item) {
            ...
            if($item['partner_id'] != 1){
                $list[$i]['is_partner'] = 1;
                //$list[$i]['partner_info'] = Partner::findOne(['user_id'=>$item['partner_id']]);
                if(empty($list[$i]['partner_info'])){
                    $list[$i]['partner_info'] = User::find()->where(['id'=>$item['partner_id']])->select('nickname name,mobile')->asArray()->one();
                }
            }
            ...
            $list[$i]['partner_benifit'] = $item['pay_price']
                                            - $item['first_price']
                                            - $item['second_price']
                                            - $item['third_price']
                                            - $item['rebate']
                                            - $item['pt_amount'];
            if($item['is_pay'] == 0 || $item['pay_time'] >= strtotime(Lashou::DAI_FA_START_TIME)){
                //如果不是到店自提 公司发货 则扣除成本
                if($item['is_offline'] == 0){
                    $shipping_fee = max($item['partner_shipping_fee'], $item['express_price']);
                    $list[$i]['partner_benifit'] -= $shipping_fee;

                    if ($item['pay_time'] <= VERSION2_SART_TIME){
                        $list[$i]['partner_benifit'] -= Lashou::getGoodsCostOld($item['id']);
                    }else{
                        $list[$i]['partner_benifit'] -= Lashou::getGoodsCost($item['id']);
                    }
                }
            }

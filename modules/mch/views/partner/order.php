                            ...
                            <div class="form-group row w-20">
                                <div class="col-5">
                                    <select class="form-control" name="keyword_1">
                                        ...
                                        <option value="4" <?= $_GET['keyword_1'] == 4 ? "selected" : "" ?>><?= PARTNER_LABEL ?>ID</option>
                                    </select>
                                </div>
                                <div class="col-7">
                ...
                <table class="table table-bordered bg-white">
                    <tr>
                        <td colspan="5">
                            ...
                            <?php if ($order_item['is_partner'] == 1): ?>
                                <span style="color: red" class="mr-5">　<?= PARTNER_LABEL ?>：<?= $order_item['partner_info']['name'] ?></span>
                                <span style="color: red" class="mr-5">　<?= PARTNER_LABEL ?>电话：<?= $order_item['partner_info']['mobile'] ?></span>
                            <?php else: ?>
                                <span style="color:red" class="mr-5">　平台订单</span>
                            <?php endif; ?>
                            <span class="mr-5">用户邮费：<?= sprintf("%.2f",$order_item['express_price']) ?></span>
<!--                            <span><?= PARTNER_LABEL ?>补交邮费：--><?php //if($order_item['partner_id'] > 1):?><!----><?php //$fee = max($order_item['partner_shipping_fee'],(($order_item['express_price']))); echo sprintf("%.2f",($fee - ($order_item['express_price']) )) ?><!----><?php //else:?><!----><?//=sprintf("%.2f",0)?><!----><?//endif;?><!--</span>-->
                    ...
                    <tr>
                        ...
                        <td class="order-tab-2">
                            <?php if ($order_item['is_partner'] == 1): ?>
                            <div"><?= PARTNER_LABEL?>：<b style="color: red"><?= $order_item['partner_info']['name'] ?></b></div>
                            <?php else: ?>
                                <div style="color:red">平台订单</div>
                            <?php endif; ?>
                            ...
                            <?php else: ?>
<!--                                <div>运费：--><?//= sprintf("%.2f",max($order_item['partner_shipping_fee'],(($order_item['express_price'])))) ?><!--元</div>-->
                                <div>运费：<?php if($order_item['is_offline'] == 1):?><?= sprintf("%.2f",0)?><?php else:?><?= sprintf("%.2f",max($order_item['partner_shipping_fee'],(($order_item['express_price'])))) ?><?php endif;?>元</div>
                            <?php endif; ?>
                        ...
                        <td class="order-tab-3">
                            ...
                            <div><?= PARTNER_LABEL?>收益：<b style="color: red"><?php if ($order_item['partner_id'] > 1 ):?><?= number_format($order_item['partner_benifit'],2)?><?php else:?><?=sprintf("%.2f",0)?><?endif;?></b></div>
                        </td>
                        <td class="order-tab-4">
                            ...
                            <?php if ($order_item['is_pay'] == 1 && $order_item['is_send'] == 0): ?>
                                <div>
                                    待发货人：
                                    <?php if ($order_item['pay_time'] > strtotime('2018-07-26')): ?>
                                        <span class="badge badge-default">平台发货</span>
                                    <?php else: ?>
                                        <?php if ($order_item['partner_id'] > 1): ?>
                            ...
                            <?php if ($order_item['is_send'] == 1): ?>
                                <div>
                                    发货人：
                                    <?php if ($order_item['send_user_id'] > 1): ?>
                                        <span class="badge badge-success"><?php $nickname = \app\models\User::find()->where(['id'=>$order_item['send_user_id']])->select('nickname')->asArray()->one()?><?= $nickname['nickname'].'('.$order_item['send_user_id'].')' ?></span>
                                    <?php else: ?>
                                        <span class="badge badge-default">平台发货</span>
                                    <?php endif; ?>
                                </div>

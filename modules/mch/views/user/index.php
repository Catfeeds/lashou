                ...
                <tr>
                    ...
                    <td>
                        ...
                        <select class="form-control" name="level">
                            <option value="-1" <?= $u['level'] == -1 ? "selected" : "" ?>>普通用户</option>
                            <!--                                --><?php //foreach ($level as $value): ?>
                            <!--                                    <option-->
                            <!--                                        value="--><?//= $value->level ?><!--" --><?//= ($value->level == $user->level) ? "selected" : "" ?><!-->--><?//= $value->name ?><!--</option>-->
                            <!--                                --><?php //endforeach; ?>
                            <option value="1" <?= $u['level'] == 1 ? "selected" : "" ?>>一星用户</option>
                            <option value="2" <?= $u['level'] == 2 ? "selected" : "" ?>>二星用户</option>
                        </select>
<!--                        --><?//= $u['level'] ? $u['l_name'] : '普通用户' ?>
                        <?= $u['is_clerk'] == 1 ? "（核销员）" : "" ?>
                    </td>
                    <td>
                        <a class="btn btn-sm btn-link"
                           href="<?= $urlManager->createUrl(['mch/order/index', 'keyword' => $u['id'],'keyword_1'=>4]) ?>"><?= User::getCount($u['id']) ?></a>
                    </td>
                    <td>
                        <a class="btn btn-sm btn-link"

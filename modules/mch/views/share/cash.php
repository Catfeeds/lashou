                        ...
                        <div class="dropdown float-right ml-2">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                                ...
                                <a href="javascript:void(0)" class="btn btn-secondary batch dropdown-item"
                                   data-url="<?= $urlManager->createUrl(['mch/share/batchcash']) ?>" data-content="是否批量审核"
                                   data-type="0">批量审核</a>
                                <a href="javascript:void(0)" class="btn btn-warning batch dropdown-item"
                                   data-url="<?= $urlManager->createUrl(['mch/share/batchcash']) ?>" data-content="是否批量打款"
                                   data-type="1">批量打款</a>
                                <a href="javascript:void(0)" class="btn btn-danger batch dropdown-item"
                                   data-url="<?= $urlManager->createUrl(['mch/share/batchcash']) ?>" data-content="是否批量驳回"
                                   data-type="2">批量驳回</a>
...
<!--                    <td>--><?//= $value['user_id'] ?><!--</td>-->
                    <td data-toggle="tooltip" data-placement="top" title="<?= $value['nickname'] ?>">
                        <span
                            style="width: 150px;display:block;white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"><img
                                ...
                                style="width: 30px;height: 30px;margin-right: 10px;"><?= $value['nickname'] ?>
                    ...
                    <?php if ($value['pay_time'] > 0): ?><td><div><?= date('Y-m-d H:i', $value['pay_time']); ?></div> <span><?php if ($value['type'] == 0 && $value['u_partner'] !=1): ?><?= $value['price']*0.9 ?>元 <?php elseif($value['type'] == 0 && $value['u_partner'] == 1):?><?= $value['price'] ?>元<?php elseif ($value['type'] == 3): ?><?= $value['price'] ?>元<?php endif; ?> <strong style="color: palevioletred;font-size: small"><?php if($value['u_partner'] == 1):?><?= PARTNER_LABEL?><?php endif;?></strong></span></td> <?php else: ?><td>未到账</td> <?php endif; ?>

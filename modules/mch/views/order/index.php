    ...
    <div class="panel-body">
                    ...
                    <div flex="dir:left">
                            ...
                            <div class="form-group row w-20">
                                <div class="col-5">
                                    <select class="form-control" name="keyword_1">
                                        ...
                                        <option value="2" <?= $_GET['keyword_1'] == 2 ? "selected" : "" ?>>用户昵称</option>
                                        <option value="3" <?= $_GET['keyword_1'] == 3 ? "selected" : "" ?>>收货人</option>
                                        <option value="5" <?= $_GET['keyword_1'] == 5 ? "selected" : "" ?>>发货人ID</option>
                                    </select>
                                </div>
                                <div class="col-7">
                        ...
                        <div class="dropdown float-right ml-2">
                            <button class="btn btn-secondary dropdown-toggle" type="button"
                                ...
                                <?php if($_GET['send_type']==='1'): ?><?= PARTNER_LABEL?>发货
                                <?php elseif($_GET['send_type']==='0'): ?>公司发货
                                <?php elseif($_GET['send_type']==''): ?>发货方式
                                <?php else: ?>
                                <?php endif; ?>
                            </button>
                            <div class="dropdown-menu" style="min-width:8rem"
                            >
                                <a class="dropdown-item" href="<?= $urlManager->createUrl(['mch/order/index']) ?>">发货方式</a>
                                <a class="dropdown-item" href="<?= $urlManager->createUrl(['mch/order/index','send_type' => 0]) ?>">公司发货</a>
                                <a class="dropdown-item" href="<?= $urlManager->createUrl(['mch/order/index','send_type' => 1]) ?>"><?= PARTNER_LABEL?>发货</a>
        ...
        <div class="mb-4">
            <ul class="nav nav-tabs status">
                ...
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $is_official == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge([$_GET['r']], $condition, ['is_official' => 1])) ?>">平台订单</a>
                </li>
                <li class="nav-item">
                    <a class="status-item  nav-link <?= $is_official_bundle == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge([$_GET['r']], $condition, ['is_official_bundle' => 1])) ?>">套餐订单</a>
                </li>
            </ul>
        </div>
                ...
                <table class="table table-bordered bg-white">
                    <tr>
                        <td colspan="5">
                            ...
                            <span class="mr-5">用户昵称：<?= $order_item['nickname'] ?></span>
                            <span class="mr-5">用户ID：<?= $order_item['user_id'] ?></span>
                            <span class="mr-5">用户邮费：<?= sprintf("%.2f",$order_item['express_price']) ?></span>
                            <?php if($order_item['is_offline'] == 0):?>
                                <span><?= PARTNER_LABEL?>补交邮费：<?php if($order_item['is_offline'] == 1):?><?= sprintf("%.2f",0)?><?php else:?><?php if($order_item['partner_id'] > 1):?><?php $fee = max($order_item['partner_shipping_fee'],(($order_item['express_price']))); echo sprintf("%.2f",($fee - ($order_item['express_price']) )) ?><?php else:?><?=sprintf("%.2f",0)?><?endif;?><?endif;?></span>
                            <?endif;?>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="order-tab-2">
                            ...
                            <?php if ($order_item['express_price_1']> 0): ?>
                                <div>运费：<?= $order_item['express_price_1'] ?>元</div>
                                <div class="text-danger">包邮，运费减免</div>
                            <?php else: ?>
                                <div>运费：<?php if($order_item['is_offline'] == 0):?><?= sprintf("%.2f",max($order_item['partner_shipping_fee'],(($order_item['express_price'])))) ?><?php elseif($order_item['is_ziti'] == 0):?><?= sprintf("%.2f",$order_item['express_price'])?><?php else:?><?= sprintf("%.2f",0)?><?php endif;?>元</div>
                            <?php endif; ?>
                        ...
                        <td class="order-tab-4">
                            ...
                            <?php endif; ?>
                            <?php if ($order_item['is_pay'] == 1 && $order_item['is_send'] == 0): ?>
                                <div>
                                    待发货人：
                                    <?php if ($order_item['partner_id'] > 1 && $order_item['is_offline'] == 1): ?>
                                        <span class="badge badge-success"><?php $nickname = \app\models\User::find()->where(['id'=>$order_item['partner_id']])->select('nickname')->asArray()->one()?><?= $nickname['nickname'].'('.$order_item['partner_id'].')' ?></span>
                                    <?php else: ?>
                                        <span class="badge badge-default">平台发货</span>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
                            <?php if ($order_item['is_send'] == 1): ?>
                        ...
                        <td class="order-tab-5">
                            ...
                            <?php if ($order_item['apply_delete'] == 1): ?>
                                ...
                                <?php endif; ?>
                            <?php else: ?>
                                <?php if (($order_item['is_pay'] == 1 || $order_item['pay_type'] == 2) && $order_item['is_confirm'] != 1 && ($order_item['is_offline'] == 0 || $order_item['partner_id'] <= 1)): ?>
                        ...
                        <form class="send-form" method="post">
                            ...
                            <div class="is-true-express">
                                ...
                                <input class="form-control" placeholder="请输入收件人邮编" type="text" autocomplete="off"
                                       name="post_code">
                                <label><a class="print" href="javascript:">使用电子面单</a></label>
                                <div><label><a class="do-print" href="javascript:;" target="_blank">打印面单</a></label></div>
                                <label>快递单号</label>
...
<script>
    ...
    $(document).on("click", ".send-btn", function () {
        ...
        var url = "<?=$urlManager->createUrl(['mch/order/do-print'])?>";
        $('.send-modal .do-print').attr('href', url + "&order_id=" + order_id);

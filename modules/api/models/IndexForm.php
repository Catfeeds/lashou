...
class IndexForm extends Model
{
    ...
    public function search()
    {
        ...
        //是否领取新手红包
        $res = LsBonusLog::findOne(['uid' => $user_id, 'type' => 1 ,'extension_id' => 1]);
        if(!empty($res)){
            $hongbao = Null;
        }else{
        ...
        $result = Ssdsplayer::findOne(['uid'=>$user_id]);
        if(!empty($result)){
            $dasai = null;
        }else{
            $dasai = [];
        ...
        //平台推荐 热卖推荐
        $recomment_platform_form = new GoodsListForm;
        $recomment_platform_form->cat_id = 52;
        $recomment_platform = $recomment_platform_form->search();
        $recomment_platform = ["list" => $recomment_platform['data']['list'], "title" => "平台推荐"];

        $recomment_hot_form = new GoodsListForm;
        $recomment_hot_form->cat_id = 53;
        $recomment_hot = $recomment_hot_form->search();
        $recomment_hot = ["list" => $recomment_hot['data']['list'], "title" => "热卖推荐"];

        array_splice($module_list,2,0,[["name" => "huanqiu"]]);
        ...
        //是否签到
        $can_qiandao = true;
        $today_qiandao = LsQiandaoLog::find()->where(['user_id' => $user_id, 'add_date' => intval(date("Ymd"))])->one();
        //print_r([1, LsQiandaoLog::find()->where(['user_id' => $user_id, 'add_date' => intval(date("Ymd"))])->createCommand()->getRawSql()]);
        if($today_qiandao){
            $can_qiandao = false;
        }
        ...
        return [
            ...
            'data' => [
                ...
                'hongbao' => $hongbao,
                'dasai'=>$dasai,
                'recomment_platform' => $recomment_platform,
                'recomment_hot' => $recomment_hot,
                'guessLike' => $guessLike,
                'can_qiandao' => $can_qiandao,

...
class CashForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['pay_type','is_partner'],'in','range'=>[0,1,2,3]],
            [['bank_name'],'string'],
        ];
    }
    ...
    public function save()
    {
        ...
        if ($this->validate()) {
            ...
            if($this->is_partner != 1){
                ...
                if ($user->partner_amount < $this->cash) {
                    return [
                        'code' => 1,
                        'msg' => '提现金额不能超过' . PARTNER_LABEL . '剩余金额'
                    ];
                }
            ...
            $cash->pay_time = 0;
            $cash->is_partner = $this->is_partner?$this->is_partner:0;
            if ($cash->save()) {
                FormId::addFormId([
                ...
                $cash->is_partner?$user->partner_amount -= $this->cash:$user->price -= $this->cash;
                if (!$user->save()) {
                    $t->rollBack();
                    return [
                        'code' => 1,
...
//            }
            ...
            if ($this->cash < 10) {
                return [
                    'code' => 1,
                    'msg' => '当前账户余额'.$user->ls_price.'元，还差'.(10 - $user->ls_price). '元即可提现~'
                ];
            }
            if ($user->ls_price < $this->cash) {
                return [
                    'code' => 1,
                    'msg' => '提现金额不能超过剩余金额'
                ];
...
//            }
            ...
            if ($cash->save()) {
                ...
                $user->ls_price -= \Yii::$app->user->identity->ls_price;
                if (!$user->save()) {
                    $t->rollBack();
                    return [
                        'code' => 1,

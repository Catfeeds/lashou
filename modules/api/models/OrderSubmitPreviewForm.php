...
//        }
        ...
        //获取商城发货方式
        $res['data']['send_type'] = time() >= strtotime(Lashou::DAI_FA_START_TIME) ? 1 : 0;
        if(isset($_REQUEST['test_offline'])){
            $res['data']['send_type'] = 0;
        }
        $res['data']['send_type'] = 1;
        // 获取 店铺积分使用规则
        ...
        $shipping_fee = $goods_amount < 150 ? 6 : ($goods_amount < 298 ? 8 : 0);
        $shipping_fee = $free_shipping ? 0 : $shipping_fee;

        $res['data']['goods_amount'] = $goods_amount;
        $res['data']['express_price'] = $shipping_fee;

        $res['data']['shop_list'] = [Lashou::getShop($this->user_id)];
        $res['data']['shop_list_uid'] = $this->user_id;

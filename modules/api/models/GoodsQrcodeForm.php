...
class GoodsQrcodeForm extends Model
{
    ...
    public function lsShareQr(){
        $res_data = [];
        $goods = Goods::findOne(101);
        if (!$goods)
            return [
                'code' => 1,
                'msg' => '商品不存在',
       ...
       if (false && file_exists($goods_pic_save_path . $goods_pic_save_name)) {
             return [
                 'code' => 0,
                 'data' => [
                     'pic_url' => $pic_url . '?v=' . time(),
        ...
        //https://api.anmeila.com.cn/statics/images/ssds/wx_share_ssds.png
        $player_source = lsPlayerSource::find()->where(['user_id' => $this->user_id, 'type' => 1])->orderBy('id desc')->one();
        $res_data['query'] = lsPlayerSource::find()->where(['user_id' => $this->user_id, 'type' => 1])->orderBy('id desc')->createCommand()->getRawSql();
        if($player_source){
            $member->avatar_url = $player_source->val;
        }else{
            $res_data['url'] = $member->avatar_url;
            ...
            if($res_data['last'] == '/132'){
                ...
                $res_data['new'] .= '/0';
                $member->avatar_url = $res_data['new'];
            }
        }
        $goods_pic_path = $this->saveTempImage($member->avatar_url);
        if (!$goods_pic_path)
            return [
                'code' => 1,
                'msg' => '获取商品海报失败：商品图片丢失',
        ...
        //获取小程序码图片
        $wxapp_qrcode_file_res = $this->getQrcode($goods->id, $this->user_id,1);
        if ($wxapp_qrcode_file_res['code'] == 1) {
            unlink($goods_pic_path);
            return [
                'code' => 1,
        ...
        $name_size = 30;
        $name_width = 670;
        $localTime1 =  strtotime(date('Y-m-d',time()));
        $localTime2 =  strtotime('2018-06-08');
        $localDay = round(($localTime1-$localTime2)/3600/24);
        //商品名称处理换行
        $name = $this->autowrap($name_size, 0, $font_path, '我正在参加安美拉首届第2代变型记瘦身大赛，共 30 天，目前第'.$localDay.'天，动动手指，给我投票，一起瓜分49万，请大家为我点赞加油 !', $name_width, 4);
        //加商品名称
       $editor->text($goods_qrcode, $name, $name_size, 40, 750, new Color('#333333'), $font_path, 0);
    ...
    private function getQrcode($goods_id, $user_id,$is_shareQr = 0)
    {
        ...
        if($is_shareQr == 1){
            $data = json_encode([
                'scene' => "{$user_id}",
                'page' => 'pages/ls-toupiao-share/ls-toupiao-share',
                'width' => 240,
            ]);

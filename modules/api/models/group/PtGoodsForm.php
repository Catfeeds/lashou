...
class PtGoodsForm extends Model
{
    ...
    public function getList()
    {
        ...
        $query = PtGoods::find()
              ...
              ->andWhere(['<>', 'id', ActivityConfig::PINGTUAN_ID]);
        if ((int)$cid){
            // 分类
    ...
    public function getInfo()
    {
        ...
        if($this->gid == ActivityConfig::PINGTUAN_ID && (!in_array($user_id, ActivityConfig::getCanShareActivityUsers()) && empty($_REQUEST['share_user_id']))){
            $activity_user = ActivityUser::findOne(['user_id' => $user_id]);
        ...
        //wooran
        $share_user_id = empty($_REQUEST['share_user_id']) ? 0 : intval($_REQUEST['share_user_id']);
        if(!empty($share_user_id) && $this->gid == ActivityConfig::PINGTUAN_ID){
            $wr_user_id = \Yii::$app->user->identity->id;
            ...
            $wr_transaction = \Yii::$app->db->beginTransaction();
            try{
                //投票日志
                $entry = new ActivityUser();
                $entry->user_id = $wr_user_id;
                $entry->add_time = date('Y-m-d H:i:s');
                $entry->add_type = 2;
                $entry->save();

                $wr_transaction->commit();
            }catch(\Exception $e){
                $wr_transaction->rollback();
            }
        }
        ...
        $need_mobile = false;
        if($this->gid == ActivityConfig::PINGTUAN_ID){
            $activity_user = ActivityUser::findOne(['user_id' => $user_id]);
            if(empty($activity_user) || $activity_user->mobile == ""){
                $need_mobile = true;
            }
        }
        return [
            ...
            'data'  => [
                ...
                'groupList'      => $groupList,
                'show_group' => $this->gid != ActivityConfig::PINGTUAN_ID,

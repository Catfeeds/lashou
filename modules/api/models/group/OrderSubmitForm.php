...
//        ]);
        ...
        foreach ($goods_list as $k => $val) {
            ...
            if($val->goods_id == ActivityConfig::PINGTUAN_ID){
                $hasOrder = PtOrder::find()->alias('o')
                    ->leftJoin(PtOrderDetail::tableName() . ' as g', 'g.order_id = o.id')
                    ->where([
                        'o.user_id' => $this->user_id,
                        'g.goods_id' => ActivityConfig::PINGTUAN_ID
                    ])
                    ->one();
                if(!empty($hasOrder)){
                    if($hasOrder->status != 4){
                        return [
                            'code' => 1,
                            'msg' => '您有未完成拼团',
                        ];

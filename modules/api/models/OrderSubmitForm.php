...
class OrderSubmitForm extends Model
{
    ...
    public function save()
    {
        ...
        foreach ($goods_list as $goods){
            $ls_goods_amount += $goods->price;
            if($goods->goods_id == LsSsds::SHIPPING_FEE_GOODS){
                $free_shipping = true;
            }

            $goods_weight += $goods->weight * $goods->num;
        }
        $ls_shipping_fee = $ls_goods_amount < 150 ? 6 : ($ls_goods_amount < 298 ? 8 : 0);
        $express_price = $free_shipping ? 0 : $ls_shipping_fee;

        $gen_partner_shipping_fee = PartnerShippingFee::genShippingFee($goods_weight, $address);
        $partner_shipping_fee = $gen_partner_shipping_fee['shipping_fee'];
        $partner_shipping_fee_log = $gen_partner_shipping_fee['log'];

        if ($this->offline == 1){
            $express_price = 0;

            $partner_shipping_fee = 0;
            $partner_shipping_fee_log = [];

            foreach ($goods_list as $goods){
                if($goods->goods_id == 199 || $goods->goods_id == 205) {
                    $t->rollBack();
                    return [
                        'code' => 1,
                        'msg' => '订单提交失败，该产品不允许自提',
        ...
        foreach ($goods_list as $goods){
            if($goods->goods_id == 199 && $goods->num != 2) {
                $t->rollBack();
                return [
                    'code' => 1,
                    'msg' => '订单提交失败，216商品数量必须为2',
                ];
            }

            if($goods->goods_id == 205 && $goods->num != 1) {
                $t->rollBack();
                return [
                    'code' => 1,
                    'msg' => '订单提交失败，套餐商品数量必须为1',
                ];
            }

            if($goods->goods_id == LsSsds::SHIPPING_FEE_GOODS && $goods->num != 1) {
                $t->rollBack();
                return [
                    'code' => 1,
                    'msg' => '订单提交失败，数量必须为1',
        ...
        if (!empty($goods_list)) {//平台自营下的单子
            ...
            if($user->is_partner==1){
                ...
                $order->from_partner_id = $user->id;
                $order->partner_grab_time = time();
            }else if(!empty($user->parent_id)){

                $parent_id = $this->getPartner($user->parent_id);
                if(!empty($parent_id)){

                    $order->partner_id = $parent_id;
                    $order->from_partner_id = $parent_id;
                    $order->partner_grab_time = time();
                }
            ...
            $member = User::findOne($this->user_id);
            $memberSx = Ssdsplayer::findOne(['uid'=>$this->user_id]);

            foreach ($goods_list as $g_k => $g_v) {

                    if($g_v->goods_id == 205 ) {

                        if ($member->is_distributor >= 1 || !empty($memberSx)) {
                ...
                if($g_v->goods_id == 199 || $g_v->goods_id == 205) {

                        if($this->payment != 0 && !LsSsds::getIsSsdsV2()){
                            $t->rollBack();
                            return [
                                'code' => 1,
                                'msg' => '订单提交失败，该产品不允许使用余额支付',
                            ];
                        }
            ...
            foreach ($goods_list as $g_k => $g_v){
                ...
                if((($g_v->goods_id == 199 || $g_v->goods_id == 205) && !LsSsds::getIsSsdsV2()) || $g_v->cat_id == 24 || $g_v->goods_id == LsSsds::SHIPPING_FEE_GOODS){
                    $order->partner_id = 1;
                    $order->from_partner_id =  1;
                    $order->partner_grab_time = time();
                    break;
                }
            ...
            $order->partner_shipping_fee = $partner_shipping_fee;
            $order->partner_shipping_fee_log = json_encode($partner_shipping_fee_log);
    ...
    //获取最近合伙人
    private  function getPartner($parent_id){

        $parentMember = User::findOne($parent_id);
        if(empty($parentMember)) return 0;

        if($parentMember->is_partner != 1){
            $parent_id = $parentMember->parent_id;
             return $this->getPartner($parent_id);
        }
         else
    ...
    private function getGoodsListByGoodsInfo($goods_info)
    {
       ...
       $goods_cat = GoodsCat::findOne(['goods_id' => $goods->id]);


        $goods_item = (object)[
            'goods_id' => $goods->id,
            'cat_id' => $goods_cat->cat_id,
            'goods_name' => $goods->name,
            'goods_pic' => $goods_pic,
            'num' => $goods_info->num,

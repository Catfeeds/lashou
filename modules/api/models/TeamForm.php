...
class TeamForm extends Model
{
    ...
    public function getOrder()
    {
        ...
        foreach ($list as $index => $value) {
            ...
            /*if($value['parent_s1_rebate'] > 0 && $value['parent_id'] == $this->user_id){
                $new_list[$index]['share_status'] = "一星收益返现";
                $new_list[$index]['share_money'] = $value['parent_s1_rebate'];
            }
            if($value['parent_s2_rebate'] > 0 && $value['parent_id_1'] == $this->user_id){
                $new_list[$index]['share_status'] = "二星收益返现";
                $new_list[$index]['share_money'] = $value['parent_s2_rebate'];
            }*/
    ...
    public function getTeam()
    {
        ...
        $query = "SELECT
                u.nickname, u.addtime,u.parent_id, u.id, u.avatar_url,u.level,count(al.id) count,
        ...
        foreach ($list as $index => $value) {
            ...
            if($value['level']=='-1'){
                $list[$index]['levelstr'] = '普通用户';
            }else if($value['level']=='1'){
                $list[$index]['levelstr'] = '一星用户';
            }else if($value['level']=='2'){
                $list[$index]['levelstr'] = '二星用户';
            }
            $isprice  = \Yii::$app->db->createCommand("SELECT  SUM(pay_price) price FROM `hjmallind_order` WHERE  user_id=".$value['id']." AND is_pay = 1 AND is_delete=0 ;")->queryAll();
            $ispricenum  = \Yii::$app->db->createCommand("SELECT  count(*) num FROM `hjmallind_order` WHERE  user_id=".$value['id']." AND is_pay = 1 AND is_delete=0 ;")->queryAll();
            if(!empty($isprice[0]['price'])){
                $list[$index]['isprice'] = $isprice[0]['price'];
                $list[$index]['num'] = $ispricenum[0]['num'];
            }else{
                $list[$index]['isprice'] =0;
                $list[$index]['num'] =0;
            }

            $join_ssds = Ssdsplayer::find()->where(['uid' => $value['id']])->one();
            if(empty($join_ssds)){
                $list[$index]['nickname'] .= ' 〖☆未参赛〗';
            }else{
                $list[$index]['nickname'] .= ' 【★已参赛】';
            }
        }
        $data['list'] = $list;

...
class UserController extends Controller
{
    ...
    public function actionSetting(){
        ...
        $user->avatar_url = \Yii::$app->request->post('avatar');
        if($user->save()){
    ...
    public function actionIndex()
    {
        ...
        if($nowDay >= strtotime(Lashou::DAI_FA_START_TIME)){
            ...
            if(count($order_ids) > 0){
                ...
                //旧版本
                $order_ids_old = Order::find()
                    ->where(['in', 'id', $order_ids])
                    ->andWhere(['<=','pay_time',VERSION2_SART_TIME])
                    ->select('id')
                    ->column();
                $order_goods_cost_old = OrderDetail::find()->alias('og')
                    ->leftJoin(Goods::tableName() . ' as g', 'og.goods_id = g.id')
                    ->where(['in', 'og.order_id', $order_ids_old])
                    ->sum('og.num * g.old_cost_price');

                $order_ids_new = Order::find()
                    ->where(['in', 'id', $order_ids])
                    ->andWhere(['>','pay_time',VERSION2_SART_TIME])
                    ->select('id')
                    ->column();
                $order_goods_cost_new = OrderDetail::find()->alias('og')
                    ->leftJoin(Goods::tableName() . ' as g', 'og.goods_id = g.id')
                    ->where(['in', 'og.order_id', $order_ids_new])
                    ->sum('og.num * g.cost_price');

                $now_shouyi -= $shipping_fee + $order_goods_cost_old + $order_goods_cost_new;
            }

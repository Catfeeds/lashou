...
//                [':addtime' => $over_day, ':store_id' => $this->store_id]);
        ...
        $profit_time = $time - ($this->store->after_profit_time * 86400);
        ...
        */
        ...
        /**
         * Wynters 已作废 2018-08-07 01:20
         * 合伙人订单暂存处理
         *
         */
/*        if(!empty($this->store->markettime)){

            //获取合伙人暂存最大时间
            $markettime = $this->store->markettime * 3600;
            $orderEndList = Order::find()->where(['is_pay'=>1,'is_send'=>0,'is_price'=>0])
                ->andWhere(['!=','partner_id',1])
                ->andWhere(['!=','partner_id',0])
                ->asArray()->all();

            foreach ($orderEndList as $o_k => $o_v){
                $partner_grab_timeEnd = $orderEndList[$o_k]['partner_grab_time'] + $markettime;
                //file_put_contents('/data/wwwroot/api.anmeila.com.cn/behaviors/1-'.$orderEndList[$o_k]['partner_id'].'.txt',$partner_grab_timeEnd);
                if($partner_grab_timeEnd < time())
                    Order::updateAll(['partner_id'=>0],['id'=>$orderEndList[$o_k]['id']]);
            }
        ...
        $order_list = Order::find()->alias('o')
            ->where([
                'and',
                ['o.is_delete' => 0, 'o.is_send' => 1, 'o.is_confirm' => 1,'is_price'=>0, 'o.store_id' => 1],
                ['<=', 'o.confirm_time', $profit_time],
            ])
            ->select(['o.*'])->groupBy('o.id')
            ->offset(0)->asArray()->all();
...
//            $this->give_integral_ms($value['id']);
    ...
    private function share_money($id)
    {
        ...
        if(!empty($order->partner_id)){
            ...
            $partnerMember = User::findOne($order->partner_id);
            //计算合伙人分润
            $amount = $order->pay_price -  $order->rebate - $order->first_price - $order->second_price - $order->pt_amount - $order->third_price;
            if($order->pay_time >= strtotime(Lashou::DAI_FA_START_TIME)){
                $shipping_fee = max($order->partner_shipping_fee, $order->express_price);
                $amount -= $shipping_fee;

                //如果不是到店自提 公司发货 则扣除成本
                if($order->is_offline == 0){
                    $amount -= Lashou::getGoodsCost($order->id);
                }
            }
            //发放合伙人佣金
            $partnerMember->partner_total_amount += $amount;
            $partnerMember->partner_amount += $amount;

            $partnerMember->save();
            UserShareMoney::set($amount,$order->partner_id,$order->id,0,7,$order->store_id,0);
            $order->is_price = 1;
            $order->save();
        }
        ...
        if($order->rebate > 0){
           ...
           /* $parent1Member = User::findOne($user['parent_id']);
            if($order->parent_s1_rebate > 0){
                $parent1Member->total_price += $order->parent_s1_rebate;
                $parent1Member->price += $order->parent_s1_rebate;
                $parent1Member->save();
                UserShareMoney::set($order->parent_s1_rebate,$parent1Member->id,$order->id,0,5,$order->store_id,0);
            }

            if($order->parent_s2_rebate > 0){
                $parent2Member = User::findOne($parent1Member['parent_id']);
                $parent2Member->total_price += $order->parent_s2_rebate;
                $parent2Member->price += $order->parent_s2_rebate;
                $parent2Member->save();
                UserShareMoney::set($order->parent_s2_rebate,$parent2Member->id,$order->id,0,6,$order->store_id,0);
            }*/
    ...
    public function share_money_1($order_id, $type = 0)
    {
        ...
        if ($this->share_setting->level >= 1) {
            ...
            $model = UserShareMoney::findOne([
                'store_id' => $this->store_id,
                'user_id' => $order_share->parent_id_1,
                'order_id' => $order_id,
                'type' => 0,
                'is_delete' => 0,
                'order_type' => 2
        ...
        if ($this->share_setting->level >= 2) {
            ...
            $model = UserShareMoney::findOne([
                'store_id' => $this->store_id,
                'user_id' => $order_share->parent_id_2,
                'order_id' =>$order_id,
                'type' => 0,
                'is_delete' => 0,
                'order_type' => 2
        ...
        if ($this->share_setting->level >= 3) {
            ...
            $model = UserShareMoney::findOne([
                'store_id' => $this->store_id,
                'user_id' => $order_share->parent_id_3,
                'order_id' => $order_id,
                'type' => 0,
                'is_delete' => 0,
                'order_type' => 2

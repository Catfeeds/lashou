...
Page({
  recommendModule:{
    ...
    related_goods_list:[],
    hot_goods_list:[],
    data: { type: "related", goods_list: [] },//related 相关宝贝 hot.热销推荐
    init: function (page, related_goods_list, hot_goods_list){
      this.page = page;
      this.related_goods_list = related_goods_list;
      this.hot_goods_list = hot_goods_list;

      this.setType("related");

      var context = this;
      page.changeRecommendType = function(e){
        console.log("recommed type", e);
        context.setType(e.currentTarget.dataset.recType);
      }
    },
    setType:function(recommendType){
      var context = this;

      context.data.type = recommendType;
      if (recommendType == "related"){
        context.data.goods_list = context.related_goods_list;
      }else{
        context.data.goods_list = context.hot_goods_list;
      }
    ...
    flashModule:{
      ...
      setCurrentIndex:function(index){
        var current = index + 1;
        var total = this.pic_list.length;
        this.page.setData({
          flash: {current:index+1, total:total},
        });
      }
    },
    ...
    onLoad: function (options) {
      ...
      if (!app.isLogin()) {
        ...
        getApp().track("产品详情页面", options);
        console.log(wx.getSystemInfoSync());
        console.log("goods detail", options);
        share_count = 0;
        p = 1;
        ...
        app.request({
            ...
            success: function (res) {
                if (res.code == 0) {
                    ...
                    console.log(res);
                    if (res.data.id == 205)
                         res.data.attr_group_list[1].attr_list[0].attr_name = res.data.parent_name;
                    page.setData({
                        goods: res.data,
                    ...
                    page.flashModule.init(res.data.pic_list, page);

                    page.recommendModule.init(page, res.data.related_goods_list, res.data.hot_goods_list);

                    console.log('after bind attr map null', page.data.goods);
                }
                if (res.code == 1) {
                    wx.showModal({
                        title: "提示",
    ...
    selectDefaultAttr: function () {
        ...
        console.log("select default attr", page.data.attr_group_list);
        page.setData({
  ...
  addCart: function () {
      var page = this;
      getApp().track("加入购物车", { goods_id: page.data.id, goods_number: page.data.form.number});
      if (page.data.id == 199 && page.data.form.number < 2 ) {
            wx.showToast({
              title: '参赛产品起拍数量不得少于2件',
              icon: 'none'
            })
    ...
    buyNow: function () {
        getApp().track("立即购买", { goods_id: this.data.id, goods_number: this.data.form.number });
        this.submit('BUY_NOW');
    },
    ...
    submit: function (type) {
        ...
        if (!page.data.show_attr_picker) {
            console.log("show attr picker", page.data.goods);
            page.setData({
    ...
    bindCurrentAttrMap: function (currentAttrMap){
      var page = this;
      var quick_goods = page.data.goods;
      var quick_goods_attr = JSON.parse(quick_goods.attr);
      ...
      if(typeof currentAttrMap == 'undefined' || currentAttrMap == null){
        currentAttrMap = [];
        for (var i = 0; i < attr_group_list.length; i++){
          currentAttrMap.push(-1);
        }
        //console.log('undefined fix', currentAttrMap);
      }

      for(var i=0; i<attr_group_list.length; i++){
        var attr_list = attr_group_list[i].attr_list;
        for(var j=0; j<attr_list.length; j++){
          var targetMap = getTargetAttrMap(i, attr_list[j].attr_id);

          attr_group_list[i].attr_list[j].disabled = getDisable(targetMap);
        }
      }

      console.log("attr_group_list disable status ", attr_group_list);
      page.setData({ attr_group_list: attr_group_list});
      ...
      function getDisable(attrMap){
        ...
        console.log("==================");
        console.log('targetMap', attrMap);

        for(var i=0; i<quick_goods_attr.length; i++){
            var product = quick_goods_attr[i];
            console.log('product', product);

            //默认命中，只要有一项不相等则为非命中
            var targeted = true;
            for (var j = 0; j < product.attr_list.length; j++){
              if (attrMap[j] != product.attr_list[j].attr_id && attrMap[j] != -1){
                targeted = false;
                break;
              }
            }
            console.log('product targeted', targeted ? "target" : "not target");

            //如果命中，且库存大于0，则可以使用
            console.log('product.num', product.num);
            if (targeted && product.num > 0){
              disabled = false;
              break;
            }
        }
    ...
    attrClick: function (e) {
      ...
      getApp().track("选择规格", { attr_group_id: attr_group_id, attr_id: attr_id});
        ...
        for (var i in attr_group_list) {
          var checkedId = -1;
          for (var j in attr_group_list[i].attr_list) {
            console.log("attr item", attr_group_list[i].attr_list[j]);
            if (attr_group_list[i].attr_list[j].checked) {
              checkedId = attr_group_list[i].attr_list[j]['attr_id'];
            }
          }
          currentAttrMap.push(checkedId);
        }
        console.log('currentAttrMap', currentAttrMap);
        page.bindCurrentAttrMap(currentAttrMap);
    ...
    onShow: function () {
        ...
        setTimeout(function(){
          console.log('after 10s', page.data.goods);
        },5000);
    },
    ...
    onShareAppMessage: function () {
        ...
        var res = {
            path: "/pages/goods/goods?id=" + this.data.id + "&user_id=" + user_info.id + "&from_share=1",
            /*success: function (e) {
                console.log(e);
                share_count++;
                if (share_count == 1)
        ...
        console.log("goods share is ", res);
        return res;
    },
    play: function (e) {
    ...
    hide: function (e) {
        ...
        this.flashModule.setCurrentIndex(e.detail.current);
    },

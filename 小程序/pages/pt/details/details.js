...
Page({
    ...
    onLoad: function (options) {
            app.pageOnLoad(this);
            getApp().track("拼团详情页", options);
            var parent_id = 0;
            var user_id = options.user_id;
      console.log("options555=>" + JSON.stringify(options), typeof decodeURIComponent(options.scene));
            var scene;
            ...
            if (user_id != undefined) {
              ...
              console.log('options 1', options, parent_id);
              ...
              console.log('options 2', options, parent_id);
            }
            app.loginBindParent({ parent_id: parent_id });
      console.log('options 3', options, parent_id);
            this.setData({
                id: options.gid,
            });
            // console.log(options);return;
            options.share_user_id = parent_id;
    ...
    onShareAppMessage: function () {
      ...
      var title = page.data.goods.name;
      var pingtuan_id = wx.getStorageSync("pingtuan_id");
      console.log('share group ', pingtuan_id, page.data.goods.id);
      if (page.data.goods.id == pingtuan_id) {
        title = user_info.nickname + "邀请您参与全球首届粉丝狂欢节";
        getApp().request({
          url: api.activity.share,
          data: { share_type: 2 },
          success: function (res) {
            console.log("用户分享拼图详情页", res);
          },
        });
      }
    ...
    getGoodsInfo: function (e) {
      console.log("get goods info ", e);
        ...
        var from_sence = "";
        if (typeof e.from != 'undefined' && e.from != null){
          from_sence = e.from;
        }
        ...
        app.request({
            ...
            method: "get",
          data: { gid: gid, from: from_sence, share_user_id: e.share_user_id},
            success: function (res) {
                if (res.code == 0) {
     ...
     */
    buyNow: function () {
      if (typeof this.data.need_mobile != 'undefined' && this.data.need_mobile){
        return;
        }
  ...
  getPhoneNumber: function (e) {
    getApp().track("粉丝活动 允许获取手机号", null);
    var context = this;

...
Page({
  ...
  onLoad: function (options) {
      ...
      this.setData({
        pingtuan_id:wx.getStorageSync('pingtuan_id')
      });
  ...
  onShareAppMessage: function (options) {
    ...
    var title = "快来" + page.data.goods.price + "元拼  " + page.data.goods.name;
      if(page.data.goods.id == pingtuan_id){
        getApp().request({
          url: api.activity.share,
          data: {share_type:3},
          success: function (res) {
            console.log("用户分享参团页", res);
          },
        });
  ...
  getInfo: function (e){
      var oid = e.oid;
        ...
        app.request({
            ...
            method: "get",
          data: { oid: oid, share_user_id: e.share_user_id },
            success: function (res) {
                if (res.code == 0) {
 ...
 */
  ...
  clickGoodsMeta:function(){
    return;
    var pingtuan_id = wx.getStorageSync("pingtuan_id");
    console.log("click goods meta ", pingtuan_id, this.data.goods.id);
    if(this.data.goods.id == pingtuan_id){
      wx.navigateTo({
        url: '/pages/ls-activity/ls-activity',
      })
    }
  }
})
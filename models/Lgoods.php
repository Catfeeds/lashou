 ...
 */
class Lgoods extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
...
//            [['store_id', 'cat_id', 'status', 'addtime', 'is_delete', 'sort', 'virtual_sales', 'individual_share', 'freight', 'use_attr', 'share_type', 'quick_purchase', 'hot_cakes', 'mch_id', 'goods_num', 'member_discount', 'anmeila_id'], 'integer'],
    ...
    public function getAttrInfo($attr_id_list)
    {
        ...
        foreach ($attr_rows as $i => $attr_row) {
            ...
            if (!array_diff($attr_id_list, $key)) {
                ...
                if (!$attr_rows[$i]['status'])
                    $attr_rows[$i]['status'] = $this->status;
                return $attr_rows[$i];
            }
        }
        return null;
    ...
    public function getCheckedAttrData()
    {
        ...
        foreach ($attr_data as $i => $attr_data_item) {
            ...
            if (!isset($attr_data[$i]['pic']))
                $attr_data[$i]['pic'] = '';
            if (!isset($attr_data[$i]['status']))
                $attr_data[$i]['status'] = 1;
            foreach ($attr_data[$i]['attr_list'] as $j => $attr_list) {
                $attr_group = $this->getAttrGroupByAttId($attr_data[$i]['attr_list'][$j]['attr_id']);
